...
!function() {
  ...
  function i(a) {
    var b = {};
    return (
      (b.x = a.clientX - window.innerWidth / 2 || 0),
      (b.y = a.clientY - window.innerHeight / 2 || 0),
      (b.fi =
        Math.atan(0 === b.x ? 1 / 0 : -b.y / b.x) +
        (b.x < 0 ? Math.PI : -b.y < 0 ? 2 * Math.PI : 0)),
      (b.s = 45 * Math.sin(2 * b.fi) / 100),
      (b.x /= 100),
      (b.y /= 100),
      (b.r =
        2 *
  ...
  function j(a) {
    for (var b = {}, c = 0; c < e.length; c++)
      b[e[c]] = +a.getAttribute("data-speed-" + e[c]) || 0;
    return b;
  }
  function k(a, b) {
    var c = {};
    return (
      (c.x = a.x * b.x + "px"),
      (c.y = a.y * b.y + "px"),
      (c.scale = 1 + a.r * b.scale),
      (c.opacity = 1 - a.r * Math.abs(b.opacity)),
      (c.rotate = 100 * -a.s * a.r * b.rotate + "deg"),
      c
    );
  }
  function l(a, c) {
  ...
  function m(a) {
    if (!d) {
      ...
      for (var b, c, g, e = i(a), f = 0; f < h.length; f++)
        (g = h[f]), (b = j(g)), (c = k(e, b)), l(g, c);
    }
  }
